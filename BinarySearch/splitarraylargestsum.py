...
def splitArray(nums : List[int],k : int) -> int :
    ...
    def getcutsCount(upperbound_sum):
        cuts_count = 0
        cur_sweetness = 0

        for s in nums:
            cur_sweetness += s
            #prev is smaller or equal to hence the current value is considered to be a part of max hence greater
            if cur_sweetness > upperbound_sum:
                #print(cur_sweetness)
                cuts_count += 1
        ...
        return cuts_count + 1
    #Lower bound is supposed to be max value
    left  = max(nums)
    right = sum(nums)
    # MINIMAL SWEETNESS[above function] [MAX total SUM] = right
    #print(right)
    while(left < right):
        mid = left + (right - left ) // 2 #min
