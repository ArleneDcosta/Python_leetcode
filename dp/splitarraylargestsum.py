...
def splitArray(nums : List[int],m : int) -> int :
    # i will be the lenght of the no
    # j will be the partition
    n = len(nums)
    ...
    for i in range(1,n+1):
        ...
        for j in range(1,m+1):
            #k = current length
            for k in range(i):
                # Have to compute with previous group since the max with the current group
                cur_max = max(pre_sum[i] - pre_sum[k],dp[k][j-1])
